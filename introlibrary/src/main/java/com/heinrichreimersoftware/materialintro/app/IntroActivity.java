...
public class IntroActivity extends AppCompatActivity {
    ...
    private void findViews(){
        ...
        buttonNext.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(adapter.getSlide(position).onNextButtonClick()) {
                    nextSlide();
                    hideKeyboard(IntroActivity.this);
                }
            }
        });
        buttonBack.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if(adapter.getSlide(position).onPrevButtonClick()) {
    ...
    public void nextSlide(boolean ignoreRestriction) {
        int currentItem = pager.getCurrentItem();
        if (currentItem > adapter.getCount() - 1) finishIfNeeded();

        if (ignoreRestriction || canGoForward(currentItem, true)) {
    ...
    public void previousSlide(boolean ignoreRestriction) {
        int currentItem = pager.getCurrentItem();
        if (currentItem <= 0) return;

        if (ignoreRestriction || canGoBackward(currentItem, true)) {
            smoothScrollPagerTo(--currentItem);
        }
        else {
    ...
    public void lockSwipeIfNeeded() {
        if (position < getCount()) {
            pager.setSwipeLeftEnabled(!lock && canGoForward(position, false));
            pager.setSwipeRightEnabled(!lock && canGoBackward(position, false));
        }
    }
